...
class MainWindow:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self._status = tk.StringVar(value="按 + 開始框選截圖")
    ...
    def _on_add(self) -> None:
        ...
        self.root.after(120, self._start_snip)

    def _start_snip(self) -> None:
        try:
            image = snip(self.root)
        finally:
            self.root.deiconify()
            self.root.lift()
    ...
    def _on_copy(self) -> None:
        if not self._captures:
            ...
            messagebox.showinfo("COPY", "請先用 + 擷取至少一張截圖。")
            return
