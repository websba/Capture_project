...
def test_a_ten_row_scroll_adds_ten_rows_not_a_duplicate_band() -> None:
    page = _page(9)
    reel = Reel.started(_frame(page, 0))

    grown = ingest(reel, _frame(page, 10))

    assert isinstance(grown, Grew)
    assert grown.added_rows == 10
    assert grown.reel.strip.height == 210
    assert grown.reel.strip.tobytes() == _span(page, 0, 210)


def test_a_near_flat_ramp_scrolls_without_a_duplicate_band() -> None:
    pixels = bytearray()
    for y in range(PAGE_HEIGHT):
        grey = (y * 255) // (PAGE_HEIGHT - 1)
        pixels += bytes((grey, grey, (y * 3) & 0xFF)) * PAGE_WIDTH
    page = Image.frombytes("RGB", (PAGE_WIDTH, PAGE_HEIGHT), bytes(pixels))
    reel = Reel.started(_frame(page, 40))

    grown = ingest(reel, _frame(page, 60))

    assert isinstance(grown, Grew)
    assert grown.added_rows == 20
    assert grown.reel.strip.height == 220
    assert grown.reel.strip.tobytes() == _span(page, 40, 260)


def test_an_anchor_that_matches_but_the_overlap_does_not_is_refused() -> None:
    page = _page(10)
    other = _page(11)
    reel = Reel.started(_frame(page, 0))

    fake = _view(other, 0).copy()
    tail = page.crop((0, VIEW_HEIGHT - 48, PAGE_WIDTH, VIEW_HEIGHT))
    fake.paste(tail, (0, 50))

    result = ingest(reel, Frame.of(fake))
...
def test_overlap_hint_rows_follow_the_anchor_and_shrink_on_short_frames() -> None:
    assert overlap_hint_rows(200) == 48
    assert overlap_hint_rows(90) == 30
    assert overlap_hint_rows(3) == 1
