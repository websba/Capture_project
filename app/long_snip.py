...
from app.vstitch import Detached, Frame, FrameHash, Reel, ingest, overlap_hint_rows
...
WHEEL_DELTA = 120
DEFAULT_WHEEL_NOTCHES = 3
MIN_WHEEL_NOTCHES = 1
MAX_WHEEL_NOTCHES = 20
...
def _hole_centre(rect: ScreenRect) -> wintypes.POINT:
    return wintypes.POINT(rect.left + rect.width // 2, rect.top + rect.height // 2)


def _owned_by(hwnd: int, root_hwnd: int) -> bool:
    while hwnd:
        if hwnd == root_hwnd:
            return True
        hwnd = int(_user32.GetParent(hwnd) or 0)
    return False


def _target_under(rect: ScreenRect, overlay_hwnd: int = 0) -> int:
    target = int(_user32.WindowFromPoint(_hole_centre(rect)) or 0)
    if not target or (overlay_hwnd and _owned_by(target, overlay_hwnd)):
        return 0
    return target
...
def _pack_lparam(x: int, y: int) -> int:
    ...
    return packed - 0x100000000 if packed >= 0x80000000 else packed


def _focus_target_under(rect: ScreenRect, overlay_hwnd: int = 0) -> int:
    """The wheel follows keyboard focus when Windows hover-scroll is off."""
    target = _target_under(rect, overlay_hwnd)
    if target:
        _user32.SetForegroundWindow(target)
    return target


def _nudge_target(rect: ScreenRect, notches: int, overlay_hwnd: int = 0) -> None:
    """Ask the window in the hole to scroll. Pixel distance is the target's choice."""
    target = _focus_target_under(rect, overlay_hwnd)
    if not target or notches == 0:
        return
    step = WHEEL_DELTA if notches > 0 else -WHEEL_DELTA
    centre = _hole_centre(rect)
    lparam = _pack_lparam(centre.x, centre.y)
    for _ in range(abs(notches)):
        wparam = (step << 16) & 0xFFFFFFFF
        _user32.PostMessageW(target, WM_MOUSEWHEEL, wparam, lparam)
...
def _clamp_notches(value: object) -> int:
    try:
        notches = int(str(value).strip())
    except (TypeError, ValueError):
        return DEFAULT_WHEEL_NOTCHES
    return max(MIN_WHEEL_NOTCHES, min(MAX_WHEEL_NOTCHES, notches))
...
class _ChromeRing:
    ...
    def __init__(self, parent: tk.Misc, selection: Selection) -> None:
        ...
        self._notches_var = tk.IntVar(value=DEFAULT_WHEEL_NOTCHES)
    ...
    def _build(self) -> None:
        ...
        width = rect.width + 2 * SIDE_PX
        height = rect.height + 2 * FRAME_PX + OK_BAND_PX
        left = rect.left - SIDE_PX
        ...
        self._band_button(band, "OK", self._on_ok, width=6).pack(
            side=tk.RIGHT, padx=(4, 10), pady=5
        )
        self._band_button(band, "↓", self._on_scroll_down, width=3).pack(
            side=tk.RIGHT, padx=2, pady=5
        )
        self._band_button(band, "↑", self._on_scroll_up, width=3).pack(
            side=tk.RIGHT, padx=2, pady=5
        )
        tk.Spinbox(
            band,
            from_=MIN_WHEEL_NOTCHES,
            to=MAX_WHEEL_NOTCHES,
            textvariable=self._notches_var,
            width=4,
            bg=BAND_COLOR,
            fg="#FFFFFF",
            buttonbackground=BAND_COLOR,
            insertbackground="#FFFFFF",
            highlightthickness=0,
            relief=tk.FLAT,
            justify=tk.CENTER,
        ).pack(side=tk.RIGHT, padx=(10, 2), pady=6)

        win.bind("<Escape>", self._on_cancel)
        win.bind("<ButtonPress-3>", self._on_cancel)

        hole_top = FRAME_PX + band_above
        hole_bottom = hole_top + rect.height
        hint = overlap_hint_rows(rect.height)
        for x in (0, width - SIDE_PX):
            tk.Frame(win, bg=OVERLAP_COLOR, width=SIDE_PX, height=hint, bd=0).place(
                x=x, y=hole_top
            )
            tk.Frame(win, bg=OVERLAP_COLOR, width=SIDE_PX, height=hint, bd=0).place(
                x=x, y=hole_bottom - hint
            )
    ...
    def _arm(self) -> None:
        ...
        _focus_target_under(self._rect, hwnd)
    ...
    def _band_button(
        self, parent: tk.Misc, text: str, command: Callable[[], None], width: int
    ) -> tk.Button:
        return tk.Button(
            parent,
            text=text,
    ...
    def _overlay_hwnd(self) -> int:
        return _window_handle(self._top) if self._top is not None else 0

    def _notches(self) -> int:
        try:
            value: object = self._notches_var.get()
        except (tk.TclError, ValueError, TypeError):
            value = DEFAULT_WHEEL_NOTCHES
        return _clamp_notches(value)

    def _on_scroll_up(self) -> None:
        _nudge_target(self._rect, self._notches(), self._overlay_hwnd())
